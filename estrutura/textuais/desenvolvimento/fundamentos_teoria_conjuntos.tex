...
\begin{exemplo}[Classes de Diferenciabilidade]
    Na Análise Real, um teorema clássico afirma que toda função derivável é contínua, estabelecendo uma relação de inclusão entre suas classes.
...
\begin{exemplo}[Números Algébricos e Transcendentais]
    ...
    Visto que \(\pi\) e \(e\) são comprovadamente transcendentais (não pertencem a \(\mathbb{A}\)), concluímos que os algébricos são apenas uma parte estrita do plano complexo:
    \[
...
\begin{exemplo}[Corte de Dedekind]
    A notação de construção é fundamental para preencher as ``lacunas'' \ dos números racionais e construir os números reais. Podemos definir o número \(\sqrt{2}\) (que não existe em \(\mathbb{Q}\)) como o subconjunto de todos os racionais cujo quadrado é menor que 2 \parencite[p.~114]{enderton1977}. Este conjunto é chamado de Corte de Dedekind:
    \[
...
\begin{exemplo}[Decomposição da Reta Real]
    A continuidade da reta real \(\mathbb{R}\) é formado pela união de dois conjuntos com propriedades de cardinalidade fundamentalmente opostas: os Racionais (\(\mathbb{Q}\)) e os Irracionais (\(\mathbb{I}\)).
    \[
        \mathbb{R} = \mathbb{Q} \cup \mathbb{I}.
    \]
    Este é um exemplo clássico onde a união de um conjunto contável (\(\mathbb{Q}\)) com um não contável (\(\mathbb{I}\)) resulta em um conjunto não contável, ilustrando que a ``tamanho'' \ da união é dominado pelo conjunto de maior cardinalidade.
\end{exemplo}
...
\begin{exemplo}[Estrutura da Álgebra Booleana]
    O conjunto das partes \(\PP(A)\) não é apenas uma coleção de subconjuntos, ele carrega naturalmente uma estrutura algébrica. Quando equipado com as operações de união (\(\cup\)), interseção (\(\cap\)) e complemento, \(\PP(A)\) satisfaz todos os axiomas de uma Álgebra Booleana. Neste sistema, o conjunto vazio \(\varnothing\) comporta-se como o zero lógico (Falso) e o conjunto total \(A\) comporta-se como o um lógico (Verdadeiro). Para qualquer conjunto finito com \(n\) elementos, essa álgebra gera exatamente \(2^n\) elementos distintos, correspondendo a todas as combinações lógicas possíveis \parencite{givant2009}.
\end{exemplo}
...
\begin{remark}
    Embora a notação indexada \(b_i\) e a listagem explícita \(\{b_0, b_1, \dots\}\) possam sugerir uma coleção enumerável, a operação de união é definida para famílias de qualquer tamanho. A cardinalidade de \(A\) é arbitrária, podendo ser não enumerável.
\end{remark}
...
\begin{exemplo}[Exaustão de Abertos por Compactos]
    Uma propriedade crucial das uniões infinitas é que elas não preservam necessariamente a ``fechadura'' \ dos conjuntos. Considere a coleção de intervalos fechados na reta real definidos por \(A_n = [1/n, 1 - 1/n]\) para todo \(n \geq 3\). Embora cada \(A_n\) seja um conjunto fechado, a união infinita de todos eles resulta no intervalo aberto unitário:
    \[
...
\begin{exemplo}[Ou Exclusivo]
    Em Lógica Digital e Ciência da Computação, a diferença simétrica \(A + B\) é a manifestação da porta lógica \textsc{xor} (Ou Exclusivo). Se considerarmos \(A\) e \(B\) como conjuntos de bits ligados (1), a operação \(A + B\) resulta exatamente nos bits onde os valores diferem (um é 0 e o outro é 1). Esta propriedade faz da diferença simétrica a base da aritmética binária.
\end{exemplo}
...
\begin{definicao}[Relação]
    Uma relação \(R\) é um conjunto de pares ordenados. Quando \(x\) se relaciona com \(y\), escrevemos \((x,y) \in R\) ou \(xRy\).
    \label{def:relacao}
\end{definicao}
...
\begin{exemplo}[Congruência Modular]
    Uma das relações mais fecundas na matemática é a congruência módulo \(n\). Fixado um inteiro \(n > 0\), dizemos que dois inteiros \(x\) e \(y\) se relacionam, \(x \equiv y \pmod{n}\), se a diferença \(x - y\) for divisível por \(n\).
    \[
        R_n = \{(x,y) \in \mathbb{Z}^2 : \exists k \in \mathbb{Z} (x - y = k \cdot n)\}.
    \]
    Esta relação divide o conjunto dos inteiros em \(n\) classes distintas (os restos da divisão), permitindo a construção da aritmética finita ``do relógio''.
\end{exemplo}

\begin{exemplo}[A Ordem de Inclusão]
    No conjunto das partes \(\PP(S)\) de um conjunto \(S\), a inclusão de subconjuntos define uma relação fundamental.
    \[
        R_{\subseteq} = \{(A,B) \in \PP(S) \times \PP(S) : A \subseteq B\}.
    \]
    Diferente da congruência, esta relação não é simétrica (se \(A \subseteq B\) e \(A \neq B\), então \(B \not\subseteq A\)). Ela transforma \(\PP(S)\) em uma malha (\textit{lattice}), uma estrutura onde quaisquer dois elementos possuem um supremo (a união) e um ínfimo (a interseção) bem definidos.
\end{exemplo}
...
\begin{exemplo}[Espaço Coluna e Transformações Lineares]
    Na Álgebra Linear, uma transformação linear \(T: \mathbb{R}^n \to \mathbb{R}^m\) pode ser vista como uma relação funcional \(R = \{(v, w) : w = T(v)\}\). A imagem desta relação, \(\im(R)\), corresponde exatamente ao \textbf{Espaço Coluna} da matriz que representa \(T\).
    Este conjunto contém todos os vetores ``alcançáveis'' \ pela transformação. Se \(\im(R) = \mathbb{R}^m\), dizemos que a transformação é sobrejetora, garantindo que o sistema linear \(Ax = b\) tenha solução para qualquer \(b\).
\end{exemplo}

\begin{exemplo}[Resíduos Quadráticos]
    Considere a relação de ``quadrado perfeito'' \ na aritmética modular. Seja \(n\) um inteiro positivo e a relação \(Q \subseteq \mathbb{Z}_n \times \mathbb{Z}_n\) definida por \(xQy \iff x^2 \equiv y \pmod{n}\).
    A imagem desta relação, \(\im(Q)\), é o conjunto dos \textbf{Resíduos Quadráticos} módulo \(n\).
    Por exemplo, para \(n=7\), temos os quadrados: \(0^2 \equiv 0\), \(1^2 \equiv 1\), \(2^2 \equiv 4\), \(3^2 \equiv 2\), etc.
    Logo, \(\im(Q) = \{0, 1, 2, 4\}\). Os elementos fora da imagem (\(\{3, 5, 6\}\)) são chamados de não-resíduos, uma distinção fundamental para a Lei da Reciprocidade Quadrática.
\end{exemplo}
...
\begin{exemplo}[Infinito de Dedekind]
    Considere a função sucessor \(s\) nos números naturais como uma relação \(S = \{(n, n^+) : n \in \mathbb{N}\}\).
    O domínio desta relação é todo o conjunto \(\mathbb{N}\). Entretanto, pelo Segundo Axioma de Peano, o zero não é sucessor de ninguém, logo \(\im(S) = \mathbb{N} - \{0\}\).
    Calculando o \textit{field}:
    \[
        \fld(S) = \dom(S) \cup \im(S) = \mathbb{N} \cup (\mathbb{N} - \{0\}) = \mathbb{N}.
    \]
    Este exemplo ilustra uma propriedade exclusiva de conjuntos infinitos (no sentido de Dedekind): é possível que o \textit{field} seja igual ao domínio, mesmo que a imagem seja um subconjunto próprio desse domínio.
\end{exemplo}
