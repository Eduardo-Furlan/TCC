...
A base de toda a matemática moderna pode ser construída sobre o conceito de ``conjunto''  (que também chamaremos de ``coleção'', ou ``família''). \textcite{enderton1977} menciona os axiomas a seguir.

\begin{axioma}[Axioma da Extensão]\index{Axioma!da Extensão}
...
\begin{definicao}[Subconjunto]\index{Subconjunto}
...
\begin{definicao}[Igualdade de Conjuntos]\index{Conjunto!igualdade de}
    \[A = B \iff A \subseteq B \text{ e } B \subseteq A.\]
    \label{def:igualdade_conjuntos}
\end{definicao}

\input{dados/figuras/igualdade_conjuntos.tex}

\begin{exemplo}[Ordem dos Elementos e Dupla Inclusão]
    Considere os conjuntos \(A = \{1, 2\}\) e \(B = \{2, 1\}\). Para mostrarmos que \(A = B\), verificamos as duas inclusões necessárias:
    \begin{itemize}
        \item \textbf{\(A \subseteq B\):} Todo elemento de \(A\) está em \(B\), visto que \(1 \in B\) e \(2 \in B\).
        \item \textbf{\(B \subseteq A\):} Todo elemento de \(B\) está em \(A\), visto que \(2 \in A\) e \(1 \in A\).
    \end{itemize}
    Como ambas as inclusões são verdadeiras, concluímos por definição que \(A = B\). Isso também ilustra que a ordem dos elementos não altera o conjunto.
    \label{ex:ordem_elementos_dupla_inclusao}
\end{exemplo}

\begin{exemplo}[Igualdade Trivial do Conjunto Vazio]
    Seja \(A\) o conjunto formado pelas soluções naturais da equação \(x + 1 = 0\) e \(B\) o conjunto formado pelas soluções reais da equação \(x = x + 1\). Como não existe número natural que satisfaça a primeira equação, temos \(A = \varnothing\). Da mesma forma, não há número real que satisfaça a segunda equação, resultando em \(B = \varnothing\). Como ambos não possuem elementos, a dupla inclusão é satisfeita (de forma vacua), logo \(A = B\).
    \label{ex:igualdade_trivial_vazio}
\end{exemplo}

\begin{exemplo}[Distinção de Conjuntos com Vazio Aninhado]
    Considere os seguintes três conjuntos: \(\varnothing\), \(\{\varnothing\}\) e \(\{\{\varnothing\}\}\). Nenhum par formado por eles é igual, isto é, eles são conjuntos dois a dois distintos. A demonstração deste fato é deixada como exercício para o leitor.
    \label{ex:distincao_vazios_aninhados}
\end{exemplo}
...
\begin{definicao}[Subconjunto Próprio]\index{Subconjunto!próprio}
...
\begin{exemplo}[Números Surreais e Jogos]
    Em sua obra \emph{On Numbers and Games}, \textcite{conway2001} define a classe de ``Jogos'' construídos recursivamente a partir do conjunto vazio.
...
\begin{exemplo}[Classes de Diferenciabilidade]
    Na Análise Real, um teorema clássico afirma que toda função derivável é contínua\footcite[p.~89]{lima2006}, estabelecendo uma relação de inclusão entre suas classes.
    Seja \(C^0(\mathbb{R})\) o conjunto de todas as funções contínuas de \(\mathbb{R}\) em \(\mathbb{R}\), e \(C^1(\mathbb{R})\) o conjunto das funções deriváveis com derivada contínua.
    Sabemos que \(C^1(\mathbb{R}) \subseteq C^0(\mathbb{R})\).
    Entretanto, a função valor absoluto, \(f(x) = |x|\), é um elemento de \(C^0(\mathbb{R})\) que não pertence a \(C^1(\mathbb{R})\), pois não possui derivada na origem.
    A existência de tais funções garante que a inclusão é estrita:
    \[
        C^1(\mathbb{R}) \subsetneq C^0(\mathbb{R}).
    \]
\end{exemplo}

\begin{exemplo}[Números Algébricos e Transcendentais]
    Defina \(\mathbb{A}\) como o conjunto dos Números Algébricos, ou seja, o conjunto de todos os \(z \in \mathbb{C}\) que são raízes de algum polinômio não nulo com coeficientes inteiros.
    Naturalmente, \(\mathbb{A} \subseteq \mathbb{C}\).
    A questão histórica sobre se \(\mathbb{A} = \mathbb{C}\) foi resolvida com a descoberta dos números transcendentais (números que não são algébricos).
    Visto que \(\pi\) e \(e\) são comprovadamente transcendentais\footcite{niven1939} (não pertencem a \(\mathbb{A}\)), concluímos que os algébricos são apenas uma parte estrita do plano complexo:
    \[
        \mathbb{A} \subsetneq \mathbb{C}.
    \]
\end{exemplo}

\begin{definicao}[Notação de Construção de Conjuntos]\index{Notacao@Notação de construção de conjuntos}
...
\begin{remark}
    Muitas vezes, a condição \(x \in A\) pode ser incorporada à própria fórmula \(\varphi(x)\), ou o conjunto universo pode estar subentendido pelo contexto. Nesses casos, é comum simplificarmos a notação escrevendo apenas \(\{x : \varphi(x)\}\).
\end{remark}

\begin{exemplo}[Construção de Subconjunto por Propriedade]
    Considere o conjunto \(A = \{1, 2, 3, 4, 5\}\) e seja \(\varphi(x)\) a condição ``\(x\) é ímpar''. O conjunto \(B\), formado pelos elementos de \(A\) que satisfazem \(\varphi(x)\), é dado por:
    \[
        B = \{x \in A : x \text{ é ímpar}\} = \{1, 3, 5\}.
    \]
    \label{ex:construcao_subconjunto_propriedade}
\end{exemplo}

\begin{exemplo}[Construção do Conjunto Vazio por Contradição]
    ...
    \[
        \varnothing = \{x \in A : x \neq x\}.
    \]
    \label{ex:construcao_vazio_contradicao}
\end{exemplo}

\begin{exemplo}[Corte de Dedekind]
    A notação de construção é fundamental para preencher as ``lacunas''  dos números racionais e construir os números reais. Podemos definir o número \(\sqrt{2}\) (que não existe em \(\mathbb{Q}\)) por meio de um corte de Dedekind.

    Um corte de Dedekind é um conjunto \(D_i \subsetneq \Q\) tal que
    \begin{enumerate}
        \item \(D_i \neq \varnothing\)
        \item \(a \in D_i \e b < a \implies b \in D_i\)
        \item \(D_i\) não possui um maior elemento
    \end{enumerate}
    \label{def:corte_dedekind}
...
\begin{enumerate}
          ...
          O problema aqui é a palavra ``definível.''  Conseguimos definir muitos números em uma linha de texto
              {\small
                  \begin{center}
                      ...
                      O 3\textsuperscript{o} número triangular\footnote{O \(n\)-ésimo número triangular é \(a_n = 0 + 1 + 2 + \dots + n\). Para mais informações, consulte a sequência A000217 na \textcite{oeisA000217}.}, \\
                      O menor número maior que qualquer número designado com menos de  \(10^{100}\) símbolos\footnote{O ``número de Rayo''. Número ganhador do \emph{Big Number Duel}, um duelo organizado pelo Instituto de Tecnologia de Massachusetts (MIT) entre Adam Elga e Agustín Rayo. Sua definição formal pode ser encontrada em \textcite{rayo2007}.}, \\
    ...
    \item O clássico paradoxo de Russell\index{Paradoxo!de Russell}. Considere o conjunto
          \[B = \{x: x \notin x\}.\]
...
\begin{axioma}[Axioma do Vazio]\index{Axioma!do Vazio}
...
\begin{definicao}[Conjunto Vazio]\index{Conjunto!vazio}
    \(\varnothing\) é o conjunto que não contém elementos.
    \label{def:vazio}
\end{definicao}

\begin{axioma}[Axioma do Par]\index{Axioma!do Par}
    Para quaisquer dois conjuntos \(x\) e \(y\), existe um conjunto contendo apenas \(x\) e \(y\).

    \label{ax:par}
\end{axioma}

\begin{definicao}[Conjunto Par]\index{Conjunto!par}
    Para quaisquer conjuntos \(u\) e \(v\), o conjunto par \(\{u,v\}\) é o conjunto cujos únicos elementos são \(u\) e \(v\).
    \label{def:conjunto_par}
\end{definicao}

\begin{definicao}[\emph{Singleton}; Conjunto Unitário]\index{Singleton@\emph{Singleton}}\index{Conjunto!unitário}
...
\begin{axioma}[Axioma da União]\index{Axioma!da União}
...
\begin{definicao}[União Finita de Conjuntos]\index{Uniao@União!finita}
...
\begin{exemplo}[Zeros de Polinômios]
    Sejam \(P(x)\) e \(Q(x)\) dois polinômios. Se definirmos \(Z(P)\) como o conjunto de todas as raízes de \(P\), e \(Z(Q)\) como o conjunto das raízes de \(Q\), a união desses conjuntos descreve exatamente as raízes do polinômio produto \(P(x) \cdot Q(x)\).
    \[
        Z(P) \cup Z(Q) = \{x : P(x) = 0 \ou Q(x) = 0\} = Z(P \cdot Q).
    \]
    Neste caso, o conectivo lógico ``ou''  da teoria dos conjuntos é algebricamente representado pela multiplicação.
\end{exemplo}

\begin{exemplo}[Decomposição da Reta Real]
    A continuidade da reta real \(\mathbb{R}\) é formado pela união de dois conjuntos com propriedades de cardinalidade fundamentalmente opostas: os Racionais (\(\mathbb{Q}\)) e os Irracionais (\(\mathbb{I}\))\footnote{É importante mencionar que essa notação não é padrão para o conjunto dos números Irracionais, normalmente se é usado \(\R - \Q\), porém para esse exemplo, não faria sentido escrever o conjunto dos Irracionais dessa forma, por isso foi escolhido escrever como \(\mathbb{I}\).}.
    \[
...
\begin{definicao}[Conjuntos Finitos]
    ...
    \]
    Da mesma forma podemos definir \(\{x_1, x_2, x_3, x_4\} = \{x_1, x_2, x_3\} \cup  \{x_4\}\). Em geral, dados \(n\) elementos, recursivamente definimos
    \[
...
\begin{axioma}[Axioma da Potência]\index{Axioma!da Potência}
...
\begin{definicao}[Conjunto das Partes]\index{Conjunto!das partes}
...
\begin{exemplo}[\(\mathcal{P}(\{1,2,3\})\)]
    Considere o conjunto \(A = \{1, 2, 3\}\). Os elementos de \(\mathcal{P}(A)\) são todos os subconjuntos possíveis de \(A\). Para não omitir nenhum, podemos listá-los agrupados pela quantidade de elementos:
    \begin{itemize}
        \item \textbf{0 elementos:} \(\varnothing\) (o conjunto vazio é subconjunto de qualquer conjunto).
        \item \textbf{1 elemento:} \(\{1\}, \{2\}, \{3\}\).
        \item \textbf{2 elementos:} \(\{1, 2\}, \{1, 3\}, \{2, 3\}\).
        \item \textbf{3 elementos:} \(\{1, 2, 3\}\) (todo conjunto é subconjunto de si mesmo).
    \end{itemize}
...
\begin{proposicao}[Monotonicidade do Conjunto das Partes]
...
\begin{proposicao}[Inclusão de Partes Implica Inclusão de Conjuntos]
...
\begin{proposicao}[Injetividade da Operação de Partes]
...
\begin{axioma}[Axiomas de Subconjunto]\index{Axioma!de Subconjunto}
...
\input{dados/figuras/axiomas_subconjunto.tex}
...
\begin{definicao}[Interseção Finita de Conjuntos]\index{Interseccao@Interseção!finita}
...
\input{dados/figuras/intersecao_finita.tex}
...
\input{dados/figuras/complemento_relativo.tex}
...
\begin{teorema}[Inexistência do Conjunto Universo]
...
\begin{definicao}[União de Conjuntos]
    Seja \(A\) um conjunto. A união de \(A\), denotada por \(\bigcup A\), é o conjunto formado por todos os elementos que pertencem a pelo menos um dos elementos de \(A\). Simbolicamente:
    \[
...
\begin{remark}
    Quando \(A\) é uma família indexada de conjuntos, digamos \(A = \{b_i : i \in I\}\), é muito comum escrevermos \(\bigcup_{i \in I} b_i\). Embora listagens explícitas como \(A = \{b_0, b_1, b_2, \dots\}\) sugiram uma coleção enumerável, a operação de união independe do tamanho da família, sendo a cardinalidade de \(A\) arbitrária.
\end{remark}

\begin{exemplo}
    A definição de união generalizada resgata o conceito familiar da união binária. Considere um conjunto formado por apenas dois elementos, digamos \(A = \{a, b\}\). Aplicando a definição, obtemos:
    \begin{align*}
        \bigcup \{a, b\} & = \{x : x \text{ pertence a algum membro de } \{a, b\}\} \\
                         & = \{x : x \in a \text{ ou } x \in b\}.
    \end{align*}
    Este último conjunto é precisamente a definição da união binária entre \(a\) e \(b\). Logo, \(\bigcup \{a, b\} = a \cup b\).
\end{exemplo}

\begin{exemplo}
    Analisando dois casos extremos da operação de união generalizada, temos:
    \begin{itemize}
        \item \(\bigcup \{A\} = A\): Pela definição, a união busca os elementos que pertencem a algum conjunto membro de \(\{A\}\). Como o único membro de \(\{A\}\) é o próprio \(A\), segue que \(x \in \bigcup \{A\} \iff x \in A\).
        \item \(\bigcup \varnothing = \varnothing\): Para que um elemento \(x\) pertença a \(\bigcup \varnothing\), deve existir um conjunto \(B \in \varnothing\) tal que \(x \in B\). Como o conjunto vazio não possui elementos, essa condição é falsa para qualquer \(x\). Logo, a união não possui elementos.
    \end{itemize}
\end{exemplo}

\begin{exemplo}
    Uma propriedade crucial das uniões infinitas é que elas não preservam necessariamente o ``fechamento''  dos conjuntos. Considere a coleção de intervalos fechados na reta real definidos por \(A_n = [\sfrac{1}{n}, 1 - \sfrac{1}{n}]\) para todo \(n \geq 3\). Embora cada \(A_n\) seja um conjunto fechado, a união infinita de todos eles resulta no intervalo aberto unitário:
    \[
        \bigcup_{n=3}^{\infty} A_n = \bigcup_{n=3}^{\infty} \left[\frac{1}{n}, 1 - \frac{1}{n}\right] = (0, 1).
    \]
\end{exemplo}

\begin{definicao}[Interseção Arbitrária de Conjuntos]
    Definimos a interseção \(\bigcap A\) de um conjunto não vazio de conjuntos \(A\) como
    \[
        x \in \bigcap A \iff x \text{ é elemento de todo membro de } A.
    \]
    \label{def:intersecao_arbitraria}
\end{definicao}

\input{dados/figuras/intersecao_infinita.tex}

\begin{exemplo}[Colapso de Intervalos Abertos]
    Considere a família de intervalos abertos reais \(A_n = (0, \frac{1}{n})\) para cada \(n \in \mathbb{N} - \{0\}\). A interseção infinita desta família é o conjunto vazio:
    \[
        \bigcap_{n=1}^{\infty} A_n = \bigcap_{n=1}^{\infty} \left(0, \frac{1}{n}\right) = \varnothing.
    \]
    Para qualquer suposto elemento \(x > 0\) que estivesse na interseção, existe sempre um natural \(n\) tal que \(\frac{1}{n} < x\), o que implica que \(x \notin A_n\). Logo, nenhum elemento sobrevive à interseção infinita.
\end{exemplo}

\begin{teorema}[Existência e Unicidade da Interseção Arbitrária]
...
\begin{proposicao}[Inclusão de Elementos na União da Família]
...
\begin{proposicao}[União de Família de Subconjuntos]
...
\begin{proposicao}[União do Conjunto das Partes]
...
\begin{proposicao}[Inclusão de um Conjunto nas Partes de sua União]
...
\begin{proposicao}[Interseção de Conjuntos das Partes]
...
\begin{proposicao}[União de Conjuntos das Partes]
...
\begin{proposicao}[Inexistência do Conjunto de Todos os Singletons]
...
\begin{proposicao}[Não-Associatividade da Diferença de Conjuntos]
...
\input{dados/figuras/diferenca_simetrica.tex}

\begin{exemplo}[Ou Exclusivo]
    Em Lógica Digital e Ciência da Computação, a diferença simétrica \(A + B\) é a manifestação da porta lógica \textsc{xor} (Ou Exclusivo). Se considerarmos \(A\) e \(B\) como conjuntos de bits ligados (1), a operação \(A + B\) resulta exatamente nos bits onde os valores diferem (um é 0 e o outro é 1).

    \input{dados/tabelas/xor.tex}
\end{exemplo}

\begin{exemplo}[Distância entre Conjuntos]
    A diferença simétrica induz uma noção natural de ``distância''  entre dois conjuntos. Em Teoria da Medida, define-se a distância entre \(A\) e \(B\) como a medida (tamanho) de sua diferença simétrica: \(d(A,B) = \mu(A + B)\). Esta função satisfaz a desigualdade triangular, \(A + C \subseteq (A + B) \cup (B + C)\), o que significa intuitivamente que a diferença entre \(A\) e \(C\) não pode ser maior que a soma das diferenças intermediárias passando por um terceiro conjunto \(B\).
\end{exemplo}

\begin{proposicao}[Distributividade da Interseção sobre a Diferença Simétrica]
...
\begin{proposicao}[Associatividade da Diferença Simétrica]
    Para quaisquer conjuntos \(A\), \(B\) e \(C\), a diferença simétrica é associativa:
    \[ A + (B + C) = (A + B) + C \]
    \label{prop:associatividade_simetrica}
\end{proposicao}

\begin{proposicao}[Caracterizações Equivalentes da Inclusão]
...
\begin{proposicao}[Interseção da União de Famílias]
...
\begin{proposicao}[Distributividade da União sobre a Interseção de Família]
...
\begin{proposicao}[Distributividade da União com Família Arbitrária]
    A igualdade
...
\input{dados/figuras/produto_cartesiano.tex}
...
\begin{proof}
    ...
    \]
    Como \(x \in A \cup B\) e \(y \in A \cup B\), temos pela \autoref{prop:power_power} que \(\{\{x\}, \{x,y\}\} \in \PP(\PP(A \cup B))\), ou seja, \((x,y) \in \PP(\PP(A \cup B))\), assim sabemos que todos os pares ordenados \((x,y)\) com \(x \in A\) e \(y \in B\) estão em \(\PP(\PP(A \cup B))\), precisamos agora garantir que apenas pares ordenados fazem parte de \(\AAA\). Pela construção de \(\AAA\) temos que
    \[
...
\begin{definicao}[Relação]
    Uma relação \(R\) entre os conjuntos \(A\) e \(B\) é um subconjunto do cartesiano \(A \times B\). Quando \(x \in A\) se relaciona com \(y \in B\), escrevemos \((x,y) \in R\) ou \(xRy\).
    \label{def:relacao}
\end{definicao}

\input{dados/figuras/relacao.tex}

\begin{exemplo}[Congruência Modular]
    Uma relação muito importante é a congruência módulo \(n\). Fixado um inteiro \(n > 0\), dizemos que dois inteiros \(x\) e \(y\) se relacionam, \(x \equiv y \pmod{n}\), se a diferença \(x - y\) for divisível por \(n\).
    \[
        R_n = \{(x,y) \in \mathbb{Z}^2 : \exists k \in \mathbb{Z} (x - y = k \cdot n)\}.
    \]
    Esta relação particiona o conjunto dos inteiros em \(n\) classes distintas (os restos da divisão). Por exemplo, considere a congruência modulo \(7\) (\(x \equiv y \ (\mod 7)\)), ela particiona os inteiros \(\Z\) em \(7\) partes:
    \[
        \Z = \{\overline{0}, \overline{1}, \overline{2}, \overline{3}, \overline{4}, \overline{5}, \overline{6}\}.
    \]
\end{exemplo}

\begin{definicao}[Domínio]
    Nós definimos o domínio de uma relação \(R\) (\(\dom (R)\)) como
    \[
        x \in \dom (R) \iff \exists y \tq xRy.
    \]
    \label{def:dominio}
\end{definicao}

\input{dados/figuras/dominio_grafo.tex}

\input{dados/figuras/dominio_plano.tex}

\begin{exemplo}[Domínio de Convergência]
    Considere a relação \(S\) definida entre números reais \(x\) e \(y\) tal que \(y\) é a soma da série geométrica de razão \(x\):
    \[
        S = \left\{(x, y) \in \mathbb{R}^2 : \sum_{n=0}^{\infty} x^n = y\right\}.
    \]
    Embora a expressão da soma \(\frac{1}{1-x}\) possa ser calculada para qualquer \(x \neq 1\), a relação \(S\) (que exige a convergência da soma infinita) é muito mais restritiva. O domínio desta relação é limitado estritamente ao intervalo aberto:
    \[
        \dom(S) = \{x \in \mathbb{R} : |x| < 1\} = (-1, 1).
    \]
\end{exemplo}

\begin{definicao}[Imagem]
    Definimos a imagem de uma relação \(R\) (\(\im (R)\)) como
    \[
        x \in \im (R) \iff \exists t \tq tRx.
    \]
    \label{def:imagem}
\end{definicao}

\input{dados/figuras/imagem_plano.tex}

% \begin{definicao}[\textit{Field}]
%     O \textit{field} de uma relação \(R\) (\(\fld (R)\)) é definido como
%     \[
%         \fld (R) = \dom(R) \cup \im(R).
%     \]
%     \label{def:field}
% \end{definicao}
